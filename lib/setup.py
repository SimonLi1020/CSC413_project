...
def params_setup():
    ...
    json_path = os.path.join(para.model_dir, 'parameters.json')
    json.dump(vars(para), open(json_path, 'w'), indent=4)
    return para
...
def logging_config_setup(para):
    if para.file_output == 0:
        logging.basicConfig(level=para.logging_level, format='%(levelname)-8s - %(message)s')
    else:
        logging.basicConfig(level=para.logging_level, format='%(levelname)-8s - %(message)s',
                            filename=os.path.join(para.model_dir, 'progress.txt'))
        logging.getLogger().addHandler(logging.StreamHandler())
