...
def create_graph(para):
    # In TensorFlow 2.x, you don't need to create a separate graph.
    # Just instantiate your model and data generator directly.
    data_generator = create_data_generator(para)

    # Assuming PolyRNN is a subclass of tf.keras.Model
    model = PolyRNN(para, data_generator)

    return model, data_generator  # No need to return graph in TensorFlow 2.x
